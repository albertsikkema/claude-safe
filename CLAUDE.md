...
- **`CLAUDE_CONTAINER_MODE=1`** is a project-owned marker, not a Claude Code feature. The wrappers set it so anything running inside the container (user shell init, hooks, setup scripts) can detect it's executing inside the `claude-safe` sandbox and branch on that. Only its presence is checked; the value is arbitrary. Don't unset it in the wrappers.
